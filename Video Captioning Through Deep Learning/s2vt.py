...
class video_caption:
    ...
    def __init__(self,cnn_feat_dim,h_dim,batch_size,lstm_steps,video_steps,out_steps,learning_rate,epochs,frame_step):
        ...
        self.train_text_path = self.path_prj + "train/text.csv"
        self.train_feat_path = self.path_prj + "train/features/"

        self.test_text_path = self.path_prj + "test/text.csv"
        self.test_feat_path = self.path_prj + "test/features/"
        ...
        self.encode_W = tf.Variable( tf.random_uniform([dim_image, dim_hidden], -0.1, 0.1), name='encode_W')
        self.encode_b = tf.Variable( tf.zeros([dim_hidden]), name='encode_b')
...
train_feat_path = './rgb_train_features'
test_feat_path = './rgb_test_features'

train_text_path = './data/video_corpus.csv'
test_text_path = './data/video_corpus.csv'
...
def train(self):
    self.train_data = self.get_video_train_data(self.train_text_path,self.train_feat_path)
    train_captions = self.train_data['Description'].values
    self.test_data = self.get_video_test_data(self.test_data_path,self.test_feat_path)
    ...
    self.word2idx,self.idx2word,self.bias_init_vector = create_word_dict(captions, word_count_threshold=0)
...
if __name__ == '__main__':
    ...
    if sys.argv == 'test':
        model = video_caption(cnn_feat_dim,h_dim,batch_size,lstm_steps,video_steps,out_steps,learning_rate,epochs,frame_step)
        model.inference(model_path='./models/model-100')
